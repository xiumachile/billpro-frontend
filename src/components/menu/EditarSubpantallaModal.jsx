// src/components/menu/EditarSubpantallaModal.jsx
import React, { useState, useEffect } from 'react';
import { X, Layout, Save, AlertCircle } from 'lucide-react';

export default function EditarSubpantallaModal({ isOpen, onClose, subpantalla, onSave }) {
  ...
  useEffect(() => {
    if (subpantalla && isOpen) {
      setFormData({
        nombre: subpantalla.nombre || '',
        descripcion: subpantalla.descripcion || '',
        orden: subpantalla.orden || 1
      });
    } else {
      // Reset al cerrar o si subpantalla es null
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onSave(subpantalla.id, datosActualizados); // Llama a la función pasada desde el padre
      onClose(); // Cierra el modal tras éxito
    } catch (err) {
      console.error('Error al actualizar subpantalla:', err);
      setError(err.message || 'Error al actualizar la subpantalla');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl flex flex-col overflow-hidden">
        <div className="bg-gradient-to-r from-cyan-600 to-teal-600 px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Editar Subpantalla</h2>
                <p className="text-cyan-100 text-sm">Modifica la pantalla: {subpantalla.nombre}</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 flex-1 flex flex-col">
          <div className="space-y-4 flex-1">
            ...
            {/* Mensaje de error */}
            {error && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 flex items-start gap-2 mt-auto"> {/* mt-auto empuja el error al final */}
          ...
          {/* Botones */}
          <div className="flex gap-3 mt-6 pt-4 border-t"> {/* pt-4 para separar del contenido si hay scroll */}
            <button
              type="button"
              onClick={onClose}
